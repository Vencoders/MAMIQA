...
class DataLoader(object):
	...
	def __init__(self, dataset, path, img_indx, patch_size, patch_num, batch_size=1, istrain=True):
		...
		if dataset == 'live':
			self.data = folders_NSS.LIVEFolder(
				root=path, index=img_indx, transform=transforms, patch_num=patch_num)
		elif dataset == 'clive':
			self.data = folders_NSS.LIVEChallengeFolder(
				root=path, index=img_indx, transform=transforms, patch_num=patch_num)
		elif dataset == 'csiq':
			self.data = folders_NSS.CSIQFolder(
				root=path, index=img_indx, transform=transforms, patch_num=patch_num)
		elif dataset == 'koniq':
			self.data = folders_NSS.Koniq_10kFolder(
				root=path, index=img_indx, transform=transforms, patch_num=patch_num)
		elif dataset == 'fblive':
			self.data = folders_NSS.FBLIVEFolder(
				root=path, index=img_indx, transform=transforms, patch_num=patch_num)
		elif dataset == 'tid2013':
			self.data = folders_NSS.TID2013Folder(
				root=path, index=img_indx, transform=transforms, patch_num=patch_num)
		elif dataset == 'kadid10k':
			self.data = folders_NSS.Kadid10k(
				root=path, index=img_indx, transform=transforms, patch_num=patch_num)			
